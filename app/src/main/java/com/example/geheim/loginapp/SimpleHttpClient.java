...
public class SimpleHttpClient {
     ...
     */
    private static HttpClient getHttpClient(InputStream keyStoreData) {
        if (mHttpClient == null) {
            //sets up parameters
            ...
            if(keyStoreData != null) registry.register(new Scheme("https", newSslSocketFactory(keyStoreData), 443));
    ...
    private static SSLSocketFactory newSslSocketFactory(InputStream keyStoreData) {
        try {
            KeyStore trusted = KeyStore.getInstance("BKS");
//            InputStream keyStoreData = LoginLayout.getAppContext().getResources().openRawResource(R.raw.keystore);
            try {
                // Keystore password comes in place of 222222
                trusted.load(keyStoreData, "222222".toCharArray());
            } finally {
                keyStoreData.close();
            }
          /*
     ...
     * @throws Exception
     */
    public static String executeHttpPost(String url, ArrayList<NameValuePair> postParameters, InputStream keyStoreData) throws Exception {
        HttpClient client = getHttpClient(keyStoreData);
